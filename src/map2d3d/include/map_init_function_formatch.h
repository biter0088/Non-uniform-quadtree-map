...
void map_init_function(){
  if (map_init)
  {
    ...
    cout<<"地图宽度、高度、分辨率分别为:"<<width<<"、"<<height<<"、"<<setprecision(5)<<resolution<<endl;
    ...
    // 保存地图的基本信息
    mapheader_tmp.origin_x=map_.info.origin.position.x;//地图原点在坐标系中的位置
    mapheader_tmp.origin_y=map_.info.origin.position.y;
    mapheader_tmp.map_width=map_.info.width;
    mapheader_tmp.map_height=map_.info.height;
    mapheader_tmp.map_resolution=map_.info.resolution;
    if(map_info_write==1){
