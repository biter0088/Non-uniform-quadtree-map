...
void ray_cast_function(){
    ...
    double rect_height_half = 4;
    double rect_width_half = 5.0;
    ...
    double now_x = odomVec[odomVec.size()-25].pose.pose.position.x;//延迟射线投射法起作用的时间
    double now_y = odomVec[odomVec.size()-25].pose.pose.position.y;
    int x_now_car = round((18  + now_x - robot_x) / resolution) ;
    int y_now_car = round((40 + now_y - robot_y) / resolution) ;
        ...
        { 
          ...
          switch (grid_image_clone.at<unsigned char>(py+rect_height,px+rect_width)){
            case UNKNOWN:
              ...
              break;
            case PASSABLE:
              if(map_.data[x_now_car+px + (y_now_car+py)*map_.info.width] == -1 ){//这里是为了避免把占据栅格更新为空闲栅格
