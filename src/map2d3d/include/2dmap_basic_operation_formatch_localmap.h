...
void cloud2gridmap(){
  for (int i=0; i<cloudInWorld->points.size(); i++)
  {
    ...
    int x_cor = round((x_offset_total+cloudInWorld->points[i].x - robot_x) / resolution) ;// max_width、max_len为一帧点云中感兴趣点云的x、y方向上的范围
    int y_cor = round((y_offset_total+cloudInWorld->points[i].y - robot_y) / resolution) ;//
...
void through_filter_with_odom(){
  for (int o=0; o<odomVec.size(); o++)
  {
    ...
    int x_cor_car = round((x_offset_total + pos_x - robot_x) / resolution) ;//和cloud2gridmap函数对应一下
    int y_cor_car = round((y_offset_total + pos_y - robot_y) / resolution) ;
