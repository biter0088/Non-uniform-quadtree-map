...
void ray_cast_function(){
    ...
    double rect_height_half = 6;
    double rect_width_half = 6.0;
    ...
    double now_x = odomVec[odomVec.size()-15].pose.pose.position.x;//延迟射线投射法起作用的时间
    double now_y = odomVec[odomVec.size()-15].pose.pose.position.y;
    int x_now_car = round((7  + now_x - robot_x) / resolution) ;
    int y_now_car = round((10 + now_y - robot_y) / resolution) ;
